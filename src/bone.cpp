...
void Bone::Render() {
	...
	if (this->image) { // o this->image == NULL
		this->image->SetHandle(handleX * this->image->GetWidth(), handleY * this->image->GetHeight());
		Renderer::Instance().DrawImage(this->image, 0, 0, 0, this->image->GetWidth() * currentScaleX, this->image->GetHeight() * currentScaleY); 
		Renderer::Instance().TranslateMatrix(pivotX * this->image->GetWidth(), pivotY * this->image->GetHeight(), 0);
	}
	for (uint32 i = 0; i < children.Size(); i++) {
		children[i].Render();
	}
...
double Bone::Interpolate(int32 id, int32 prevId, int32 nextId, double prevVal, double nextVal) const {
	double ret = prevVal + (nextVal - prevVal) * (id - prevId) / static_cast<float>(nextId - prevId);
	return ret; 
}
